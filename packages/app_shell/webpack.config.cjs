...
const { deps } = require("../app/shared/dependencies.cjs")
...
module.exports = (env) => {
	const {
		mode = "development",
		context = Repack.getDirname("file://" + path.resolve(__dirname, "webpack.config.cjs")),
		entry = "./index.js",
		...
		reactNativePath = new URL(
			...
			"file://" + path.resolve(__dirname, "webpack.config.cjs")
		).pathname,
	} = env
	...
	return {
		...
		plugins: [
			...
			new Repack.plugins.ModuleFederationPlugin({
				name: "shell",
				exposes: {
					RecoilProvider: "./src/RecoilProvider.tsx",
				},
				shared: deps,
			}),
		],
